"""Apply the secure Growth Audit API client wiring over the reviewed static build."""
...
required_server_files = [
    ROOT / "api" / "growth-audit" / "index.php",
    ROOT / "api" / "growth-audit" / "lib.php",
    ROOT / "api" / "growth-audit" / ".htaccess",
    ROOT / ".env.example",
]
missing = [str(path.relative_to(ROOT)) for path in required_server_files if not path.is_file()]
if missing:
    raise RuntimeError("Missing Growth Audit server files: " + ", ".join(missing))

print("secure Growth Audit API layer complete")
